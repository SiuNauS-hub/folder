...
export function addPopup(calFns) {
  ...
  function updateSchedulerHeader(dateObj) {
    ...
    if (isMonthView) {
      ...
      if (calToday) calToday.textContent = `${year}년 ${month}월`;
      if (week) week.textContent = "전체 일정"; // '요일' 대신 '전체 일정' 표시
    } else {
  ...
  // 특정 날짜 일정 렌더링
  function renderMonthSchedules(dateObj) {
    const year = dateObj.getFullYear();
    ...
    const filtered = schedules.filter((s) => {
      ...
      return start <= lastDayStr && end >= firstDayStr;
    });

    if (filtered.length === 0) {
      schList.innerHTML = `<div class="empty_schedule" style="text-align:center; padding:20px; color:#888;">해당 월에 등록된 일정이 없습니다.</div>`;
      return;
    }

    filtered
    .sort((a, b) => new Date(a.start) - new Date(b.start)) // 날짜순 정렬
    .forEach((s) => schList.insertAdjacentHTML("beforeend", buildCardHTML(s)));
  }

  function refreshScheduleList() {
    if (!currentSelectedDateStr) {
      currentSelectedDateStr = new Date().toISOString().split("T")[0];
    }
    if (isMonthView) {
      const currentDate = new Date(currentSelectedDateStr + "T00:00:00");
      renderMonthSchedules(currentDate);
    } else {
  ...
  registerBtn.onclick = () => {
    ...
    save();
    if (!currentSelectedDateStr) currentSelectedDateStr = start;
    refreshScheduleList();
  ...
  document.addEventListener("monthChanged", (e) => {
    ...
    if (newDate && newDate instanceof Date) {
      isMonthView = true;
      currentSelectedDateStr = newDate.toISOString().split("T")[0];
      updateSchedulerHeader(newDate);
      renderMonthSchedules(newDate);
    } else {
      console.warn("monthChanged 이벤트에서 날짜 객체를 찾지 못했습니다.");
      renderMonthSchedules(new Date());
      updateSchedulerHeader(newDate);
    }
  });
