...
export function authorList(){
    ...
    class DocApp{
        ...
        initDoc(){
            ...
            // 파일 업로드 변경 시 파일명 표시
            if (this.fileInput && this.uploadName) {
                this.fileInput.addEventListener("change", (e) => {
                    const files = e.target.files;    
                    if (files && files.length > 0) {
                        this.uploadName.value = files[0].name;
                    } else {
                        this.uploadName.value = "첨부파일이 없습니다.";
                    }
                });
            } else {
                console.error("오류: #fileDoc 또는 #uploadNameDoc 요소를 찾을 수 없습니다!");
            }
